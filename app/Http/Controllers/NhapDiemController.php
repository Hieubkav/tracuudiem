   ...
   // P.Phong
        ...
        DB::transaction(function () use ($request, $lh_id) {
            ...
            if (is_array($danhSachSinhVien)) {
                foreach ($danhSachSinhVien as $sinhVien) {
                    ...
                    // });
                    // $updateData['svd_ghichu'] = $sinhVien['svd_ghichu'] ?? null;
                    $updateData = [
                        'svd_dulop' => $sinhVien['svd_dulop'] ?? null,
                        'svd_first' => $sinhVien['svd_first'] ?? null,
                        'svd_exam_first' => $sinhVien['svd_exam_first'] ?? null,
    ...
    public function updateBangDiemLoai0DotThi(Request $request, $lh_id)
    {
        ...
        $bangDiem = DB::table('qlsv_dotthi_bangdiem as bd')
            ...
            ->where('bd.lh_id', $lh_id)
            ->select(DB::raw('DISTINCT svd.dt_bd_id, bd.mh_id'))
            ->get();
        ...
        if (is_array($danhSachSinhVien)) {
            foreach ($danhSachSinhVien as $sinhVien) {
                ...
                foreach ($mh_id as $mh) {
                    if ($mh === 885) {
                        $dt_bd_id = $bangDiem->filter(function ($item) use ($mh) {
                            return $item->mh_id == $mh;
                        })->first()->dt_bd_id;
                        if (DB::table('qlsv_dotthi_diem')->where('dt_bd_id', $dt_bd_id)->where('sv_id', $sinhVien['sv_id'])->exists()) {
                            DB::table('qlsv_dotthi_diem')->where('dt_bd_id', $dt_bd_id)->where('sv_id', $sinhVien['sv_id'])->update([
