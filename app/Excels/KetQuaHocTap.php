...
class KetQuaHocTap
{
    ...
    private $template_diemmonhocdotthitheolopfull2022_TrungCap = 'app/excel-template/DIEMMONHOCDOTTHI_FULL2022_TRUNG_CAP.xlsx';
    ...
    public function createSheetKetQuaThiDatTN($danhSachNganhNgheSinhVien, $dxtn_id)
    {
        ...
        foreach ($danhSachNganhNgheSinhVien as $nnIndex => $nn) {
            ...
            foreach ($danhsachsinhvien as $svIndex => $sv) {
                ...
                if ($sv->svd_loai == 1) {
                    ...
                    if ($nienche != 0) {
                        ...
                        // @ttdat - Change K key previous index by 1
                ...
                if (isset($sv->toanKhoa->avg_totnghiep)) {
                    $sheet->getStyle('L' . $index)->getNumberFormat()->setFormatCode('0.0');
                    $sheet->setCellValue('L' . $index, $sv->toanKhoa->avg_totnghiep);
                    $sheet->getStyle($currentAddress)->applyFromArray($centerBorderStyle);
                }

                // XLTN
                $sheet->setCellValue('M' . $index, $sv->toanKhoa->hocLucTN);
                $sheet->getStyle($currentAddress)->applyFromArray($centerBorderStyle);


                // Ghi chú
                if ($sv->giamXltn == true) {
                    $sheet->setCellValue('N' . $index, "Giảm một mức XLTN");
                    $sheet->getStyle($currentAddress)->applyFromArray($centerBorderStyle);
                }
            ...
            foreach ($thongKeHocLucTemp as $key => $value) {
                ...
                if (!$quyChe2022 && $key == 'Trung bình khá' && isset($thongKeHocLuc['Trung bình khá'])) {
        ...
        if (!$quyChe2022 && isset($thongKeHocLuc['Trung bình khá']) && $thongKeHocLuc['Trung bình khá'] > 0) {
        ...
        if ($dotThi->dotXetTotNghiep[0]->qd_id != null) {
            $quyetDinh = QuyetDinh::find($dotThi->dotXetTotNghiep[0]->qd_id);
            if ($quyetDinh) {
                $ngayQD = Carbon::parse($quyetDinh->qd_ngay)->format('d \t\h\á\n\g m \t\h\á\n\g Y');
                $formatQD = "(Theo QĐ số: {$quyetDinh->qd_ma}, ngày {$ngayQD} của Hiệu trưởng TCĐCĐ & NN Nam Bộ)";
    ...
    public function createSheetSVDuDieuKienKhongDuDieuKien($danhSachNganhNgheSinhVien, $loai)
    {
        ...
        if ($loai == 1) {
            if ($dotThi->qd_id != "-1") {
                ...
                $formatQD = "(Kèm theo QĐ số: {$dotThi->quyetDinh->qd_ma}/QĐ-TCĐCĐ, ngày {$ngayQD} của Hiệu trưởng TCĐCĐ & NN Nam Bộ)";
            } else {
                // $formatQD = "(Theo QĐ số: ......, ngày ....... của Hiệu trưởng TCĐCĐ & NN Nam Bộ)";
                $formatQD = "(Kèm theo QĐ số:         /QĐ-TCĐCĐ, ngày 01 tháng 08 tháng 2024 của Hiệu trưởng TCĐCĐ & NN Nam Bộ)";
            }
            // (Kèm theo QĐ số:         /QĐ-TCĐCĐ, ngày 01 tháng 08 tháng 2024 của Hiệu trưởng TCĐCĐ & NN Nam Bộ)
            $sheet->setCellValue('A8', $formatQD);
    ...
    public function createSheetDanhSachXetThiTotNghiep($filterLhId, $dt_id)
    {
            ...
            foreach ($sinhVien->years as $yIndex => $year) {
                foreach ($year->semesters as $semester) {
                    ...
                    foreach ($semester->monHoc as $monHoc) {
                        ...
                        $sheet->getStyle($currentAddress)->applyFromArray($centerStyle);
                        if (!isset($final) || (isset($final) && $final < 5)) {
                            $sheet->getStyle($currentAddress)->getFill()
                                ->setFillType(Fill::FILL_SOLID)
                                ->getStartColor()->setARGB($markedColor);
                        }
        ...
        if ($tongsodieukien < 10 && $tongsodieukien > 0) {
            $tongsodieukien = '0' . $tongsodieukien;
        }
        $tongsinhvien = $danhSachSinhVien->count();
        if ($tongsinhvien < 10 && $tongsinhvien > 0) {
            $tongsinhvien = '0' . $tongsinhvien;
        }
        $sheet->setCellValue($currentAddress, 'Tổng số học sinh: ' . $tongsinhvien);
        $currentAddress = $this->nextRowAddress($currentAddress, true);
        ...
        if ($tongsokhongdieukien < 10 && $tongsokhongdieukien > 0) {
            $tongsokhongdieukien = '0' . $tongsokhongdieukien;
        }
        ...
        foreach ($danhSachGhiChuSplit as $sIndex => $notes) {
            ...
            foreach ($notes as $note) {
                ...
                $textrun = $richText->createTextRun($note['value']);
    ...
    public function createSheetMonHocDotThi($filterLhId, $dt_id)
    {
        ...
        if (!$quyChe2022 && isset($thongKeHocLuc['Trung bình khá'])) {
            $thongKeMessage[] = 'Trung bình khá: ' . $thongKeHocLuc['Trung bình khá'] . ' ' . ($nganhNghe->hdt_id == 4 ? 'SV' : 'HS');
        }
        if (isset($thongKeHocLuc['Trung bình'])) {
            $thongKeMessage[] = 'Trung bình: ' . $thongKeHocLuc['Trung bình'] . ' ' . ($nganhNghe->hdt_id == 4 ? 'SV' : 'HS');
    ...
    public function createSheetMonHocDotThiFull($filterLhId, $dt_id)
    {
        ...
        $lopHoc = $exportData['lopHoc'];
        $quyChe2022 = $lopHoc->lh_nienche;
        $isTrungCap = $lopHoc->khoaDaoTao->hdt_id == 5;
        $nienKhoa = $lopHoc->nienKhoa;
        $khoaDaoTao = $lopHoc->khoaDaoTao;
        $heDaotao = $khoaDaoTao->heDaotao;
        $nganhNghe = $khoaDaoTao->nganhNghe;
        ...
        // @ttdat - thêm điều kiện loại bỏ môn chính trị đối với quy chế 2022
        if ($quyChe2022 && $isTrungCap) {
            $additionalItems = $additionalItems->reject(function ($note) {
                return $note['key'] == 'ĐTNCT';
            });
        }
        ...
        if ($lopHoc->lh_nienche != 0) {
            // @ttdat - Bổ sung template bỏ cột XLTN cho niên chế 2022 & Trung cấp
            if ($isTrungCap) {
                $spreadsheet = IOFactory::load(storage_path($this->template_diemmonhocdotthitheolopfull2022_TrungCap));
            } else {
        ...
        foreach ($danhSachSinhVien as $sIndex => $sinhVien) {
            ...
            if ($filterSemester == 123456) {
                ...
                // @ttdat - comment fix lỗi dư cột khi xuất biểu mẫu
                // Số quyết định dự thi tốt nghiệp
                ...
                // Học lực tốt nghiệp
                // @ttdat - ẩn cột xếp loại đối với quy chế 2022 & trung cấp
                if ($quyChe2022 && $isTrungCap) {
                    // Render not thing
                } else {
                    $sheet->setCellValue($currentAddress, $sinhVien->toanKhoa->hocLucTN);
    ...
    public function createSheet($filterSemester, $filterLhId)
    {
        ...
        $thongKeMessage = [];
        if (!isset($thongKeHocLuc['Xuất sắc'])) {
            $thongKeHocLuc['Xuất sắc'] = 0;
        }
        $thongKeMessage[] = 'Xuất sắc: ' . $thongKeHocLuc['Xuất sắc'] . ' ' . ($nganhNghe->hdt_id == 4 ? 'SV' : 'HS');
        if (!isset($thongKeHocLuc['Giỏi'])) {
            $thongKeHocLuc['Giỏi'] = 0;
        }
        $thongKeMessage[] = 'Giỏi: ' . $thongKeHocLuc['Giỏi'] . ' ' . ($nganhNghe->hdt_id == 4 ? 'SV' : 'HS');
        if (!isset($thongKeHocLuc['Khá'])) {
            $thongKeHocLuc['Khá'] = 0;
        ...
        $thongKeMessage[] = 'Khá: ' . $thongKeHocLuc['Khá'] . ' ' . ($nganhNghe->hdt_id == 4 ? 'SV' : 'HS');
        if (!isset($thongKeHocLuc['Trung bình'])) {
            $thongKeHocLuc['Trung bình'] = 0;
        }
        if (!isset($thongKeHocLuc['Trung bình khá'])) {
            $thongKeHocLuc['Trung bình khá'] = 0;
        }
        if (!isset($thongKeHocLuc['Yếu'])) {
            $thongKeHocLuc['Yếu'] = 0;
        }
        if (!$quyChe2022 && !isset($thongKeHocLuc['Trung bình khá'])) {
            $thongKeHocLuc['Trung bình khá'] = 0;
        }
        ...
        $thongKeMessage[] = 'Yếu: ' . $thongKeHocLuc['Yếu'] . ' ' . ($nganhNghe->hdt_id == 4 ? 'SV' : 'HS');
        if (!isset($thongKeHocLuc['Yếu'])) {
            $thongKeHocLuc['Yếu'] = 0;
        }
        ...
        for ($n = 0; $n <= 10; $n++) {
            if ($sheet->getCell($currentAddress)->getValue() == 'NGƯỜI LẬP BẢNG') {
                break;
            } else {
                $currentAddress = $this->nextRowAddress($currentAddress, true);
            }
        ...
        $sheet->setCellValue($currentAddress, 'HỌC SINH, SINH VIÊN');
        $sheet->getStyle($currentAddress)->applyFromArray($boldCenterStyle);
        $currentAddress = $this->nextRowsAddress($currentAddress, 6);
        $sheet->setCellValue($currentAddress, 'Nguyễn Ngọc Thái');
        $sheet->getStyle($currentAddress)->applyFromArray($centerStyle);
    ...
    public function getKetQuaHocTapTheoDotThi($semester, $lhId, $dt_id, $svId = 0)
    {
        ...
        foreach ($data['danhSachSinhVien'] as $sinhVien) {
            ...
            if (isset($sinhVien->toanKhoa->avg)) {
                ...
                $sinhVien->toanKhoa->hocLucTN = getHocLuc($sinhVien->toanKhoa->avg_totnghiep, $quyChe2022);
            }
        }
        return $data;
    }
    ...
    public function getKetQuaHocTapTheoDotThiTN($semester, $lhId, $dt_id, $dxtn_id, $svId = 0)
    {
        ...
        $quyChe2022 = $lopHoc->lh_nienche == 1;
        $khoaDaoTao = $lopHoc->khoaDaoTao;
        $isTrungCap = $khoaDaoTao->hdt_id == 5;
        ...
        foreach ($data['danhSachSinhVien'] as $index => $sinhVien) {
            ...
            $diemdotthi = DB::table('qlsv_dotthi_diem as d')
                ...
                ->join('qlsv_monhoc as mh', 'mh.mh_id', '=', 'bd.mh_id')
                // @ttdat - thêm điều kiện loại bỏ môn chính trị đối với quy chế 2022
                ->where(function ($builder) use ($quyChe2022, $isTrungCap) {
                    if ($quyChe2022 && $isTrungCap) {
                        $builder->where('mh.mh_loai', '!=', 2);
                    }
                })
                ->where('d.sv_id', $sinhVien->sv_id)
                ->whereIn('d.dt_id', $numberExamreCently)
    ...
    public function getKetQuaHocTap($semester, $lhId, $svId = 0, $dxtn = false)
    {
        ...
        $svMa = request()->sv_ma;
        if ($svMa) {
            $danhSachSinhVien->where('qlsv_sinhvien.sv_ma', $svMa);
        }
        // @ttdat optional
        ...
        foreach ($danhSachSinhVien as $svIndex => $sinhVien) {
            ...
            foreach ($semesters as $seIndex => $hocKy) {
                ...
                $tinhAvg = $semester->monHoc->every(function ($monHoc) {
                    return isset($monHoc->ketQua)
                        && ($monHoc->ketQua->svd_first !== null || $monHoc->ketQua->svd_second !== null);
                });


                if ($diemRenLuyen && $diemRenLuyen->svd_final) {
                    $sumDiem = 0;
                    $sumTinChi = 0;
                    $sumDiemTl = 0;
                    $sumTinChiTl = 0;
                    $semester->monHoc->each(function ($monHoc) use (&$sumDiem, &$sumTinChi, &$sumDiemTl, &$sumTinChiTl, $semesters) {
                        if ($monHoc->mh_tichluy) {
                            ...
                            if ($monHoc->ketQua) {
                                ...
                                // @ttdat ngày 03/11 - thêm điều kiện kiểm tra học kỳ cải thiện
                                if (($monHoc->ketQua->svd_second || $monHoc->ketQua->svd_first == null)
                                    && $monHoc->ketQua->svd_second_hocky <= $semesters[count($semesters) - 1]) {
                    ...
                    if ($semester->diemRenLuyen > 0 && isset($semester->avg)) {
                        $semester->hocLuc = getHocLuc(isset($semester->avg) ? $semester->avg : 0, $quyChe2022);
                    }
                }
            ...
            foreach ($chunkSemester as $csIndex => $twoSemester) {
                ...
                $tinhDiemNam = $twoSemester->every(function ($semester) {
                    return isset($semester->diemRenLuyen) && isset($semester->avg);
                });
                ...
                $danhSachDiemMonHocTichLuyNam = $this->getDanhSachDiemMonHocTichLuy($svId, $lopHoc->kdt_id, $lopHoc->lh_id, $semesterSumTinChi);

                $danhSachDiemMonHocTichLuyNam = $danhSachDiemMonHocTichLuyNam->filter(function ($item) use ($twoSemester) {
                    // Mặc định hiển thị điểm lần 1
                    $item->score = $item->svd_first;
                ...
                $tichLuy = $danhSachDiemMonHocTichLuyNam->map(function ($item) {
                    return $item->score * $item->mh_sodonvihoctrinh;
                })->sum();

                $tinChiTichLuy = $danhSachDiemMonHocTichLuyNam->map(function ($item) {
                    return $item->mh_sodonvihoctrinh;
                })->sum();
                ...
                $diemRenLuyen = $tinhDiemNam ? number_format($twoSemester->filter(function ($semester) {
                ...
                if ($diemRenLuyen) {
                    ...
                    if ($tinhHocLucNam) {
                        $year->hocLuc = getHocLuc(isset($year->avg) ? $year->avg : 0, $quyChe2022);
                    }
                }
            ...
            $tinhDiemRenLuyen = $years->every(function ($year) {
                return isset($year->diemRenLuyen) && isset($year->avg);
            });

            $diemRenLuyen = $tinhDiemRenLuyen ? number_format($years->map(function ($year) {
            ...
            if ($diemRenLuyen) {
                ...
                if ($tinhHocLucToanKhoa) {
                    $toanKhoa->hocLuc = getHocLuc(isset($toanKhoa->avg) ? $toanKhoa->avg : 0, $quyChe2022);
                }
            }
            // End điểm toàn khóa
