...
class KetQuaHocTapSinhVien
{
    ...
    public function download($danhSachNamHoc, $danhSachLopHoc, $sinhVien, $hoc_ky)
    {
        ...
        if ($TBMonThucHanh != -1 && $TBMonLyThuyet != -1) {
            ...
            if ($hoc_ky == "123456") {
                ...
                if ($sinhVien->toanKhoa->final_xltn > $sinhVien->toanKhoa->temp_xltn) {
                    ...
                    $hocLuc = getHocLuc($trungBinhTN, $quyChe2022);
                    $templateProcessor->setValue('xeploai', $hocLuc);
        ...
        if ($hoc_ky == "123456") {
            ...
            if ($dsQuyetDinh->isEmpty()) {
                $query = <<<SQL
                select dxtn.qd_id from qlsv_dotxettotnghiep dxtn
                join qlsv_dotxettotnghiep_sinhvien sv on sv.dxtn_id = dxtn.dxtn_id
                where dxtn.dxtn_qd_trangthai = 1
                and sv.sv_id = ?
                limit 1
                SQL;
                $quyDinhId = collect(DB::select($query, [$sinhVien->sv_id]))->first();
                if ($quyDinhId) {
                    $dsQuyetDinh = QuyetDinh::where('qd_id', $quyDinhId->qd_id)->get();
                }
            }
