                                ...
                                <tbody>
                                    <tr v-for="(nn, nnIndex) in table.list" :key="'nn' +nnIndex">
                                        ...
                                        <td v-if="nn.nn_id == null && qd && nn.svd_khongdatloai != 0">
                                            <button v-if="nn.svd_dieukien == 1" class="btn btn-danger" v-on:click="showModal($event, nn.lh_id, nn.sv_id, 1)" title="Không đạt tiêu chuẩn">
                                                <i class="fa fa-minus-circle" aria-hidden="true"></i>
                                            </button>

                                            <button v-if="nn.svd_dieukien == 1" class='btn btn-warning' v-on:click="showModal($event, nn.lh_id, nn.sv_id, 2)" title="Không dự thi">
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">Nhập lý do "{{ this.editForm.model.svd_khongdatloai == 1 ? 'Không đạt tiêu chuẩn' : 'Không dự thi' }}"</h4>
                    </div>
                    ...
                    <div class="modal-footer">
                        <button type="button" v-on:click="capNhatTrangThaiSinhVien()" class="btn btn-danger">Lưu</button>
                    </div>
                </div>
            </div>
        </div>
...
import Label from '../../../../vendor/laravel/breeze/stubs/inertia-vue/resources/js/Components/Label.vue';
    ...
    export default {
        ...
        methods: {
            ...
            showModal: function (event, lh_id, sv_id, svd_khongdatloai) {
            ...
            capNhatTrangThaiSinhVien: function () {
                let svd_ghichu_detail = (this.editForm.model.svd_khongdatloai == 1 ? "Không đạt tiêu chuẩn: " : "Không dự thi: ") + this.editForm.model.dt_ghichu;

                consumer.capNhatTrangThaiSinhVien(this.dotthi.dt_id, this.editForm.model.lh_id, this.editForm.model.sv_id, this.filter.loai, svd_ghichu_detail, this.editForm.model.svd_khongdatloai)
                    .then(response => {
                        if (response.status == 200) {
