          ...
          <div class="modal-body">
            <table class="table mb-0" style="margin-bottom: 0px">
              <tr v-for="(sheet, index) in listSheet.list" :key="sheet.id">
                <td>
                  <button class="text-left btn btn-primary btn-block" v-on:click="selectSheet(index)"
...
export default {
  ...
  methods: {
    ...
    reloadSinhVien: async function () {
      ...
      var ds_sinhvienLoai1 = null;
      // Khởi tạo biến đếm
      var completedRequests = 0;
      for (let index = 0; index < mh_id_loai0.length; index++) {
        let currentMhId = mh_id_loai0[index];
        let data = await consumer.getBangDiem(lh_id, currentMhId, dt_id);
        if (ds_sinhvienLoai1 == null) {
            ds_sinhvienLoai1 = data;
        }
        for (let x = 0; x < data.data.length; x ++) {
            console.log(currentMhId);
            let sv = data.data[x];
            const svId = sv.sv_id;
            let svRef = ds_sinhvienLoai1.data.find(item => item.sv_id == svId);
            if (svRef) {
                if (currentMhId == 885) {
                    // Chính trị
                    svRef.chinhtri = sv.svd_first;
                } else if (currentMhId == 887) {
                    svRef.lythuyet = sv.svd_first;
                } else if (currentMhId == 886) {
                    svRef.thuchanh = sv.svd_first;
                }
            }
        }
        // data.data.forEach((sv) => {
