...
<script>
    ...
    const consumer = {
        getListUser: function () {
            const url = '/api/user/paginate' + window.location.search; /** Phong API */
            return axios.get(url)
                    .then(response => response.data);
        },
        getUser: function (hdt_id) {
            const url = '/api/user/' + hdt_id;/** Phong API */
            return axios.get(url)
                    .then(response => response.data);
        },
        saveOrUpdate: function (formData) {
            if (formData.user_id == null) {
                return axios.post('/api/user', formData);/** Phong API */
            } else {
                return axios.put('/api/user/' + formData.user_id, formData);/** Phong API */
            }
        },
        destroy: function (user_id) {
            return axios.delete('/api/user/' + user_id);/** Phong API */
        },
