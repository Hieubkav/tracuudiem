              ...
              <table class="table table-striped table-bordered no-margin">
                <thead>
                  <tr>
                    ...
                    <th class="text-center w-100-p">Môn học</th>
                    <th class="text-center w-90-p">Hành động</th>
                  </tr>
                </thead>
                <tbody>
...
const consumer = {
  getListNganhNghe: function (hdt_id) {
    const url = "/api/nganh-nghe/all?hedaotao=" + hdt_id; /** Phong api */
    return axios.get(url).then((response) => response.data);
  },
  getListHeDaoTao: function () {
    const url = "/api/he-dao-tao/all";/** Phong api */
    return axios.get(url).then((response) => response.data);
  },
  getListKhoaDaoTao: function () {
    const url = "/api/khoa-dao-tao" + window.location.search;/** Phong api */
    return axios.get(url).then((response) => response.data);
  },
  getKhoaDaotao: function (kdt_id) {
    const url = "/api/khoa-dao-tao/" + kdt_id;/** Phong api */
    return axios.get(url).then((response) => response.data);
  },
  saveOrUpdate: function (formData) {
    if (formData.kdt_id == null) {
      return axios.post("/api/khoa-dao-tao", formData);/** Phong api */
    } else {
      return axios.put("/api/khoa-dao-tao/" + formData.kdt_id, formData);/** Phong api */
    }
  },
  destroy: function (kdt_id) {
    return axios.delete("/api/khoa-dao-tao/" + kdt_id);/** Phong api */
  },
  duplicate: function (formData) {
    return axios.post("/api/khoa-dao-tao/duplicate/" + formData.kdt_id, formData);/** Phong api */
  },
};
...
export default {
  ...
  methods: {
    loadDanhSachNganhNghe: function () {
      ...
      var promNn = consumer.getListNganhNghe(this.filter.hedaotao).then((data) => {
    ...
    actionChangeHeDaoTaoFilter: function () {
      this.listNganhNghe.select2 = this.listNganhNghe.origin.filter((item) => {
        return item.hdt_id == this.filter.hedaotao || this.filter.hedaotao == -1;
      });
