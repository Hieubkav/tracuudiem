                                ...
                                <tbody>
                                    <tr v-for="(qd, index) in table.list.data" :key="qd.qd_id">
                                        ...
                                        <td class="text-left">
                                            ...
                                            <!-- T.Phong -->
                                            <a v-if="qd.qd_loai === 0 && qd.checklophoc === true" :href="'/lop-hoc?search=' + qd.qd_id">
                                                <button type="button" class="btn bg-purple btn-sm">
                                                    <i class="fa fa-eye"></i>
                                                </button>
                                            </a>
                                            <a v-if="qd.qd_loai === 1 && qd.dxtn_id !== null"  :href="'/dot-xet-tot-nghiep/?dxtn_id=' + qd.dxtn_id + '&tunam=' + qd.dxtn_tunam + '&dennam=' + qd.dxtn_dennam + '&chuongtrinh=' + qd.dxtn_hdt_id">
                                                <button type="button" class="btn bg-purple btn-sm">
                                                    <i class="fa fa-eye"></i>
                                                </button>
                                            </a>
                                            <a v-if="qd.qd_loai === 3 && qd.dt_id !== null"  :href="'/dot-thi?dt_id=' + qd.dt_id + '&&tunam=' + qd.dt_tunam + '&dennam=' + qd.dt_dennam + '&chuongtrinh=' + qd.dt_hdt_id">
                                                <button type="button" class="btn bg-purple btn-sm">
                                                    <i class="fa fa-eye"></i>
                                                </button>
                                            </a>
                                            <a v-if="qd.qd_loai === 2 && qd.checkusser === true" :href="'/sinh-vien?search=' + qd.qd_id ">
                                                <button type="button" class="btn bg-purple btn-sm">
                                                    <i class="fa fa-eye"></i>
                                                </button>
                                            </a>
...
<script>
    ...
    const consumer = {
        getListQuyetDinh: function () {
            const url = '/api/quyet-dinh' + window.location.search; /** Phong */
            return axios.get(url).then(response => response.data);
        },
        getQuyetDinh: function (qd_id) {
            const url = '/api/quyet-dinh/' + qd_id;/** Phong */
            return axios.get(url).then(response => response.data);
        },
        checkUsed: function (qd_id) {
            const url = '/api/quyet-dinh/check-used/' + qd_id;/** Phong */
            return axios.get(url).then(response => response.data);
        },
        saveOrUpdate: function (formData) {
            if (formData.qd_id == null) {
                return axios.post('/api/quyet-dinh', formData);/** Phong */
            } else {
                return axios.put('/api/quyet-dinh/' + formData.qd_id, formData);/** Phong */
            }
        },
        destroy: function (qd_id) {
            return axios.delete('/api/quyet-dinh/' + qd_id);/** Phong */
        }
    }
